$(document).ready(function() {
    ...
    function renderDroneList() {
        ...
        drones.forEach(function(drone) {
            var html = '<div class="drone-item-wrapper" data-id="' + drone.id + '">'
                     + '<a href="' + ctx + '/drone/stream?id=' + drone.id + '" class="drone-btn">'
    ...
    $('#btn-open-add-modal').on('click', function() {
        ...
        $('#modal-drone-id').val('');
        $('#modal-input-name').val(''); // 💡 HTML ID 매칭: modal-input-name
        $('#drone-modal').addClass('active'); // 💡 active 클래스 추가로 정상 렌더링
    });
    ...
    $(document).on('click', '.btn-edit-drone', function() {
        ...
        if (target) {
            ...
            $('#modal-input-name').val(target.name); // 💡 HTML ID 매칭
            $('#drone-modal').addClass('active');
        }
    });

    // 8. 모달 닫기
    $('#btn-modal-cancel').on('click', function() { // 💡 HTML ID 매칭: btn-modal-cancel
        $('#drone-modal').removeClass('active');
    });

    // 모달 배경 클릭 시 닫기
    $('#drone-modal').on('click', function(e) {
        if ($(e.target).is('#drone-modal')) {
            $(this).removeClass('active');
        }
    });

    // 9. 드론 저장 (신규 또는 수정)
    $('#btn-modal-save').on('click', function() { // 💡 HTML ID 매칭: btn-modal-save
        var name = $('#modal-input-name').val().trim(); // 💡 HTML ID 매칭
